<?php
$bdd = new RECEIPT();
$resulReceiptsQt = $bdd->bdd->query("SELECT COUNT(receiptsId) AS receiptsId FROM receipts WHERE receipt_status = 'Active' AND cookerId = '".$cookerId."' ") or die(mysql_error());
...
$resulReceipts = $bdd->bdd->query(" SELECT receipts.*, favorites.favoritesId AS fav, favorites.cookerId AS cookID FROM receipts LEFT JOIN favorites ON receipts.receiptsId=favorites.receiptsId WHERE ( receipts.receipt_status = 'Active' AND receipts.cookerId = '".$cookerId."') AND receipts.cookerId = favorites.cookerId GROUP BY receiptsId  DESC LIMIT {$start},{$per_page} ") or die(mysql_error());
...
<div class="container">
    ...
    <div class="col-md-12">
        ...
        <ul class="breadcrumb">            
            <li style="font-weight: bolder;" class="page_title"><i class="fa fa-fighter-jet"></i> MY FAVORITES </li>
        </ul>                
